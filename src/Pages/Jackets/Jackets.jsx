    ...
    useEffect(() => {
        const load = async () => {
            try {
                ...
                const jacketProducts = res.data.filter(p =>
                    p.category === 'Jackets' ||
                    (Array.isArray(p.categories) && p.categories.includes('Jackets'))
                );
                setProducts(jacketProducts.map(p => ({
                    id: p._id,
    ...
    return (
        <div className="dress-page">
            <div className="filter-sidebar">
                ...
                {categories.map((item, index) => (
                    <div className="filter-item" key={index}>
                        <div className="filter-left">
                            <input type="radio" name="category" id={item} className="custom-radio" defaultChecked={item === "Jackets"} />
                ...
                <input type="range" min="0" max="500" value={price} className="range-slider" onChange={(e) => setPrice(Number(e.target.value))} />
            ...
            <div className="dresses-main" style={{ flex: 1 }}>
                {loading ? (
                    <p style={{ padding: '40px', color: '#6b7280' }}>Loading jackets...</p>
                ) : products.length === 0 ? (
                    <div style={{ padding: '60px', textAlign: 'center' }}>
                        <p style={{ fontSize: '3rem' }}>🧥</p>
                        <p style={{ color: '#6b7280', marginTop: '12px' }}>No jackets added yet. Add one from the admin panel!</p>
                    </div>
                ) : (
